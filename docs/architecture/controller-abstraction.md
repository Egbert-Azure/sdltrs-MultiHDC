...
- `hard_image_offset(d, secsize, cyl, head, sec)` — byte offset past the 256-byte header. `secsize` is passed in because it is the *controller's* current bytes-per-sector, not a property of the image: WD1000 has a live sector-size register, OMTI is fixed at 512, and the Xebec runs at 256 on the TCS adapter.
