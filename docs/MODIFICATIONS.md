...
**Follow-up fix (reported by Jens Guenther, sdltrs's original author):** the same screen's "Create Hard Disk Image" action (blank-image creation) still only ever attached the new file to a WD1000 slot, regardless of which drive the user picked in its drive popup — that popup only ever listed WD1000 slots (`None`/`0`-`3`) in the first place. Fixed by adding a dedicated `hard_create_drives[]` array (`None`/`0`-`3`/`omti0`/`omti1`, 7 entries) just for this popup, kept separate from the shared `drives[]` array used elsewhere (floppy, stringy) so it doesn't change their choices, and branching to `trs_omti_attach()` when one of the two new choices is picked. `trs_create_blank_hard()` itself needed no changes — it already writes a generic Reed header valid for either controller.
