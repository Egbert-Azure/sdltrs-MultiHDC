...
Getting there also meant unifying the three hard-disk backends onto shared image handling and a single active-controller model.
...
`src/trs_xebec.c` / `.h` implements the S1410 controller core with two host-adapter interfaces onto one shared SASI state machine:

- **Holte's CP/M host adapter** at ports `0x40`–`0x42`, the same range the OMTI uses. On real hardware only one controller chip is ever installed, so `src/trs_io.c` routes that range to whichever controller is active. 512-byte sectors.
...
Both interfaces share one phase-based SASI state machine (idle → CDB → data in/out → status, with real REQ/BUSY/C-D/I-O status bits and command opcodes verified against Holte's `hd2.mac`). All three controllers read and write the same Matthew Reed `.hdv` header format (`src/reed.h`).
...
- `src/trs_hdctl.c` / `.h` — `(controller-type, unit)` dispatch, plus the single active-controller model. A real Genie IIIs is fitted with exactly one controller, so exactly one answers on the bus. The active controller is set explicitly (GUI, CLI or config) or resolved from attached images (Xebec → OMTI → WD1000) when unset.
- Drive caps follow each controller's real maximum: WD1000 = 4 (2-bit SDH drive field), OMTI and Xebec = 2 (1-bit SASI LUN).
- Config and CLI: `hardcontroller = wd1000|omti|xebec`; flags `-hard2`/`-hard3`, `-omti1`, `-xebec1`.
